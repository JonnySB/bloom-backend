...
class ChatRepository:
    ...
    def find_messages_by_userid(self, user_id):
        rows = self._connection.execute(
            "SELECT * from chats WHERE recipient_id=%s", [user_id]
        )
        messages = []
        for row in rows:
    ...
    def create(self, sender, receiver):
        ...
        existing_chat = self._connection.execute(
            query, [receiver.id, sender.id, start_date_str]
        )
        ...
        if not existing_chat:
            ...
            return self._connection.execute(
                insert_query,
                [receiver.id, receiver.message, datetime.datetime.now(), sender.id],
            )
        else:
            ...
            return self._connection.execute(
                update_query, [receiver.message, receiver.id, sender.id, start_date_str]
            )
