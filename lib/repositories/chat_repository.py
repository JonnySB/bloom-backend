...
class ChatRepository:
    ...
    def create(self,receiver, sender):
        ...
        query = '''SELECT * FROM chats WHERE recipient_id = %s AND sender_id = %s AND start_date >= %s AND end_date <= %s'''
        existing_chat = self._connection.execute(query, [receiver.id, sender.id, start_date_str, end_date_str])
        ...
        if not existing_chat:
            ...
            update_query = '''UPDATE chats SET message = array_append(message, %s) WHERE recipient_id = %s AND sender_id = %s AND start_date = %s  AND end_date = %s RETURNING *;'''
            return self._connection.execute(update_query, [receiver.message, receiver.id,  start_date_str, end_date_str,  sender.id,])
