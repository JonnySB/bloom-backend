...
def test_create_offer_and_find_all(db_connection):
    ...
    offer_1 = HelpOffer(None, 1, 'willing to help', 3.75, 'pending')
    offer_2 = HelpOffer(None, 2, 'willing to help for cheaper', 3.25, 'pending')
    repo.create_offer(offer_1)
    repo.create_offer(offer_2)
    offers = repo.all()
    assert offers == [
        HelpOffer(1, 1, 'Offering help', 50.0, 'pending'),
        HelpOffer(2, 1, 'willing to help', 3.75, 'pending'),
        HelpOffer(3, 2, 'willing to help for cheaper', 3.25, 'pending')
    ]
...
def test_find_by_id(db_connection):
    ...
    assert offer == HelpOffer(3, 2, 'willing to help for cheaper', 3.25, 'pending')
