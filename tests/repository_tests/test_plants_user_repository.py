...
def test_find_plants_by_user_id_one(db_connection):
    ...
    assert find == [
        {
            "plant": Plants(
                1, "African sheepbush", "Pentzia incana", "plant_01.png", 2
            ),
            "quantity": 5,
        }
    ]
...
def test_find_plants_by_user_id_two(db_connection):
    ...
    repository.assign_plant_to_user(4, 2, 1)  ## assigning 1 plant id 2 to user id 4.
...
def test_delete_plant_from_user(db_connection):
    repository = PlantsUserRepository(db_connection)
    repository.assign_plant_to_user(6, 1, 1)  ## assigning 1 plant id 1 to user id 6.
    repository.assign_plant_to_user(
        6, 2, 1
    )  ## assigning a second plant. 1 plant id 2 to user id 6.
